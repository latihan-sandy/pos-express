module.exports = function(app) {
    ...
    app.post('/' + routeName + '/datatable', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            connection.query("SELECT COUNT(*) as total FROM categories", function(err, rows, fields) {
                if (err) throw err;
                ...
                let sql = "SELECT ";
                sql += " categories.name as categories_name, ";
                sql += " categories.description as categories_description, ";
                sql += " categories.id as key_id, ";
                sql += " '" + action + "' as action ";

                let filter = "";
                if (searchValue && searchValue.length > 0) {
                    filter += " WHERE ";
                    filter += " categories.name LIKE '%" + searchValue + "%' OR ";
                    filter += " categories.description LIKE '%" + searchValue + "%'";
                }
                sql += " FROM categories " + filter;
    ...
    app.post('/' + routeName + '/store', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            ...
            connection.query("INSERT INTO categories SET ?", data, function(err, result, fields) {
                if (err) throw err;
    ...
    app.get('/' + routeName + '/:id', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            let id = req.param('id');
            connection.query("SELECT * FROM categories WHERE id = " + id, function(err, result, fields) {
                if (err) throw err;
    ...
    app.post('/' + routeName + '/:id/update', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            ...
            connection.query("UPDATE categories SET ? WHERE id = " + id, data, function(err, result, fields) {
                if (err) throw err;
    ...
    app.delete('/' + routeName + '/:id', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            let id = req.param('id');
            connection.query("DELETE FROM categories WHERE id = " + id, function(err, result, fields) {
                if (err) throw err;
                res.send(result);
            });
        } else {
