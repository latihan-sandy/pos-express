module.exports = function(app) {
    ...
    app.post('/' + routeName + '/datatable', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            connection.query("SELECT COUNT(*) as total FROM brands", function(err, rows, fields) {
                if (err) throw err;
                ...
                sql += " brands.description as brands_description, ";
                sql += " brands.id as key_id, ";
                sql += " '" + action + "' as action ";

                let filter = "";
                if (searchValue && searchValue.length > 0) {
                    filter += " WHERE ";
                    filter += " brands.name LIKE '%" + searchValue + "%' OR ";
                    filter += " brands.description LIKE '%" + searchValue + "%'";
                }
                sql += " FROM brands " + filter;
    ...
    app.post('/' + routeName + '/store', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            ...
            connection.query("INSERT INTO brands SET ?", data, function(err, result, fields) {
                if (err) throw err;
    ...
    app.get('/' + routeName + '/:id', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            let id = req.param('id');
            connection.query("SELECT * FROM brands WHERE id = " + id, function(err, result, fields) {
                if (err) throw err;
    ...
    app.post('/' + routeName + '/:id/update', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            ...
            connection.query("UPDATE brands SET ? WHERE id = " + id, data, function(err, result, fields) {
                if (err) throw err;
    ...
    app.delete('/' + routeName + '/:id', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            let id = req.param('id');
            connection.query("DELETE FROM brands WHERE id = " + id, function(err, result, fields) {
                if (err) throw err;
                res.send(result);
            });
        } else {
