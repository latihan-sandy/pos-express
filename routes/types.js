module.exports = function(app) {
    ...
    app.post('/' + routeName + '/datatable', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            connection.query("SELECT COUNT(*) as total FROM types", function(err, rows, fields) {
                if (err) throw err;
                ...
                sql += " types.description as types_description, ";
                sql += " types.id as key_id, ";
                sql += " '" + action + "' as action ";

                let filter = "";
                if (searchValue && searchValue.length > 0) {
                    filter += " WHERE ";
                    filter += " types.name LIKE '%" + searchValue + "%' OR ";
                    filter += " types.description LIKE '%" + searchValue + "%'";
                }
                sql += " FROM types " + filter;
    ...
    app.post('/' + routeName + '/store', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            ...
            connection.query("INSERT INTO types SET ?", data, function(err, result, fields) {
                if (err) throw err;
    ...
    app.get('/' + routeName + '/:id', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            let id = req.param('id');
            connection.query("SELECT * FROM types WHERE id = " + id, function(err, result, fields) {
                if (err) throw err;
    ...
    app.post('/' + routeName + '/:id/update', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            ...
            connection.query("UPDATE types SET ? WHERE id = " + id, data, function(err, result, fields) {
                if (err) throw err;
    ...
    app.delete('/' + routeName + '/:id', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            let id = req.param('id');
            connection.query("DELETE FROM types WHERE id = " + id, function(err, result, fields) {
                if (err) throw err;
                res.send(result);
            });
        } else {
