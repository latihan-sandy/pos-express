module.exports = function (app) {
    ...
    let pageName = "Purchases";
    let routeName = "purchases";
    ...
    app.post('/' + routeName + '/datatable', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            connection.query("SELECT COUNT(*) as total FROM transactions WHERE type_of = 0", function (err, rows, fields) {
                if (err) throw err;
                ...
                let filter = " WHERE type_of = 0";
                if (searchValue && searchValue.length > 0) {
                    filter += " AND ";
                    filter += " transactions.invoice_date LIKE '%" + searchValue + "%' OR ";
                    filter += " transactions.invoice_number LIKE '%" + searchValue + "%' OR ";
                    filter += " suppliers.name LIKE '%" + searchValue + "%'";
                }
                sql += " FROM transactions LEFT JOIN suppliers ON suppliers.id = transactions.supplier_id " + filter;
    ...
    app.get('/' + routeName + '/create', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            connection.query("SELECT * from transactions WHERE DATE(invoice_date) = DATE(NOW()) AND type_of = 0 ORDER BY id DESC LIMIT 1", function (err, result, fields) {
                if (err) throw err;
                let dateIndex = new Date().toISOString().replace('-', '').split('T')[0].replace('-', '');
                let invoice_number = "PRC." + dateIndex+".00001";
                ...
                if(result.length > 0){
                    ...
                    invoice_number = "PRC." + dateIndex + "." + next_element;
                }
                let data = {
                    type_of : 0,
                    status: 0,
    ...
    app.get('/' + routeName + '/:id', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            let id = req.param('id');
            connection.query("SELECT transactions.*, suppliers.name as supplier_name, users.username as user_username FROM transactions LEFT JOIN users ON users.id = transactions.user_id LEFT JOIN suppliers ON suppliers.id = transactions.supplier_id WHERE transactions.id = " + id, function (err, result, fields) {
                if (err) throw err;
    ...
    app.post('/' + routeName + '/:id/update', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            ...
            let supplier_id = req.param('supplier_id');
            ...
            connection.query("DELETE FROM transaction_details WHERE transaction_id = "+id, function (err, result, fields) {
                ...
                for(i = 0; i < product_id.length; i++){
                    ...
                    connection.query("UPDATE products SET stock = stock + " + parseInt(qty[i]) + ", supplier_id = "+supplier_id+" WHERE id = " + product_id[i], function (err, result, fields) {
                        if (err) throw err;
                    });
