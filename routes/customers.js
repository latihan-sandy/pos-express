module.exports = function(app) {
    ...
    app.post('/' + routeName + '/datatable', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            connection.query("SELECT COUNT(*) as total FROM customers", function(err, rows, fields) {
                if (err) throw err;
                ...
                sql += " customers.phone as customers_phone, ";
                sql += " customers.email as customers_email, ";
                sql += " customers.website as customers_website, ";
                sql += " customers.address as customers_address, ";
                sql += " customers.id as key_id, ";
                sql += " '" + action + "' as action ";

                let filter = "";
                if (searchValue && searchValue.length > 0) {
                    filter += " WHERE ";
                    filter += " customers.name LIKE '%" + searchValue + "%' OR ";
                    filter += " customers.phone LIKE '%" + searchValue + "%' OR ";
                    filter += " customers.email LIKE '%" + searchValue + "%' OR ";
                    filter += " customers.website LIKE '%" + searchValue + "%' OR ";
                    filter += " customers.address LIKE '%" + searchValue + "%'";
                }
                sql += " FROM customers " + filter;
    ...
    app.post('/' + routeName + '/store', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            ...
            connection.query("INSERT INTO customers SET ?", data, function(err, result, fields) {
                if (err) throw err;
    ...
    app.get('/' + routeName + '/:id', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            let id = req.param('id');
            connection.query("SELECT * FROM customers WHERE id = " + id, function(err, result, fields) {
                if (err) throw err;
    ...
    app.post('/' + routeName + '/:id/update', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            ...
            connection.query("UPDATE customers SET ? WHERE id = " + id, data, function(err, result, fields) {
                if (err) throw err;
    ...
    app.delete('/' + routeName + '/:id', (req, res) => {
        if (req.session.user && req.cookies.user_sid) {
            let id = req.param('id');
            connection.query("DELETE FROM customers WHERE id = " + id, function(err, result, fields) {
                if (err) throw err;
                res.send(result);
            });
        } else {
